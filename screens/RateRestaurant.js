...
const RateRestaurant = ({ route, navigation }) => {
    ...
    const getIdRestaurant = async () => {
        try {
            const listIdRestaurants = []
            for (const id of listIDproducts) {
                const data = await fetch(API_URL + `restaurants/by-dishes/${id}`, {
                    method: 'GET',
                    headers: {
                        "Content-Type": "application/json",
                    }
                }).then(res => res.json())
                listIdRestaurants.push(data._id)
            }
            return listIdRestaurants
        } catch (error) {
            console.log('get id restaurant', error);
        }
    }
    const onSubmit = async () => {
        try {
            ...
            const listIdRestaurants = await getIdRestaurant()
            for (const id of listIdRestaurants) {
                await fetch(API_URL + `evaluate/add-restaurant/${id}`, {
                    method: 'POST',
                    headers: {
                        "Content-Type": "application/json",
            ...
            console.log('Rate Restaurant ', error);
            setIsLoading(false)
        }
    }
    return (
            ...
            <KeyboardAwareScrollView className='flex-1' showsVerticalScrollIndicator={false}>
                <ImageBackground source={Pattern} className='flex-1 px-5 items-center' resizeMode='cover'>
                    <View className='w-[135] h-[135] mt-24 rounded-full border-4 border-bgrButton mb-14'>
                        <Image source={{ uri: 'https://st.depositphotos.com/25545024/52453/v/450/depositphotos_524539944-stock-illustration-restaurant-logo-design-element.jpg' }} className='w-full h-full rounded-full' resizeMode='cover' />
                    </View>
                    {/* Title */}
                    <Text className='text-center font-[BentonSans-Bold] text-2xl'>Thank You!{'\n'}
                        Enjoy Your Meal</Text>
                    {/* Sub title */}
                    <Text className='font-[BentonSans-Regular] text-[#3b3b3b] opacity-30 mt-5'>Please rate your Restaurant</Text>
                    {/* Rate */}
                    ...
                    <View className='flex-row w-full items-center mt-5 mb-8 justify-between' >
                        {/* Button Submit */}
                        {
                            isLoading ? <View className='w-[230] h-[60] bg-bgrButton rounded-2xl justify-center items-center'>
                        ...
                        {/* Button Skip */}
                        <TouchableOpacity disabled={isLoading} onPress={async () => {
                            await onClearCart()
                            navigation.reset({
                                index: 0,
                                routes: [{ name: 'Main' }]
