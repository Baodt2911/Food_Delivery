...
const Cart = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      socket.emit("cart-list", { userId, accessToken });
      socket.on("cart-list", (data) => {
        setResultsCart(data.cart);
      });
    } catch (error) {
  ...
  const SubTotal = () => {
    const resultsTotal = resultsCart?.reduce(
    ...
    <SafeAreaView className="flex-1">
      <ImageBackground source={Pattern} resizeMode="cover" className="flex-1">
        <View className="mx-[25] mt-5 flex-row justify-between items-center">
          ...
          {/* Button Order */}
          <TouchableOpacity
            disabled={resultsCart?.length === 0 || !resultsCart}
            onPress={() =>
          ...
          >
            <Text className="font-[BentonSans-Medium] text-white underline ">
              Order Now
            </Text>
            {/* Total */}
            <Text className="font-[BentonSans-Bold] text-white text-base">
              {SubTotal()} $
            </Text>
          </TouchableOpacity>
        </View>
        ...
        >
          {!resultsCart ? (
          ...
          ) : resultsCart.length === 0 ? (
            <Text className="font-[BentonSans-Medium]  text-center mt-5 text-[#3B3B3B] opacity-30">
              No products!
            </Text>
          ) : (
            resultsCart.map((data) => (
              <Swipeable
                ...
                restaurant={data.product.restaurant.name}
